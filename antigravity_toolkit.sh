...
GUIDE_FILE="$SCRIPT_DIR/ANTIGRAVITY_MAINTENANCE_GUIDE.md"
...
# ───────────────────────────────────────────────────────────────────────────
# RESTORE-RULES: Restore Global Rules
# ───────────────────────────────────────────────────────────────────────────
cmd_restore_rules() {
    ...
    # Extract Global Rules from Guide
    if [ -f "$GUIDE_FILE" ]; then
        echo "📄 Extracting Global Rules from Guide..."
        
        # Extract content between ```markdown and ``` in Global Rules Backup section
        sed -n '/^## 📋 Global Rules Backup$/,/^---$/p' "$GUIDE_FILE" | \
        sed -n '/^```markdown$/,/^```$/p' | \
        sed '1d;$d' > "$GEMINI_DIR/GEMINI.md"
        ...
        if [ -s "$GEMINI_DIR/GEMINI.md" ]; then
            ...
            echo -e "${RED}❌ Failed to extract Global Rules${NC}"
            echo "Please manually copy from ANTIGRAVITY_MAINTENANCE_GUIDE.md"
        fi
    else
...
cmd_full() {
    ...
    # Step 2: Create new .gemini and restore rules
    echo "═══════════════════════════════════════════════════════════════════"
